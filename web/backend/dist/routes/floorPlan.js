...
const PRISMA_TO_ROOM_TYPE = Object.fromEntries(Object.entries(ROOM_TYPE_TO_PRISMA).map(([k, v]) => [v, k]));
const USER_STATUS_TO_PRISMA = {
    available: 'available',
    assigned: 'assigned',
    'checked-in': 'checked_in',
};
const PRISMA_TO_USER_STATUS = {
    available: 'available',
    assigned: 'assigned',
...
router.get('/', async (req, res) => {
    try {
        ...
        if (planId) {
            ...
            const plan = await prisma_1.default.floorPlan.findUnique({
                where: { id: planId },
                include: {
                    floors: {
                        orderBy: { order: 'asc' },
            ...
            const storeData = {
                ...
                floors: plan.floors.map((floor) => ({
                    ...
                    entryPoints: floor.entryPoints.map((ep) => ({
                        id: ep.id,
                        x: ep.x,
                        y: ep.y,
                        label: ep.label ?? undefined,
                    })),
                    corridors: floor.corridors.map((c) => ({
                        id: c.id,
                    ...
                    evacuationRoutes: floor.evacuationRoutes.map((er) => ({
                        id: er.id,
                        points: er.points,
                    })),
                })),
        ...
        const plans = await prisma_1.default.floorPlan.findMany({
    ...
    try {
        ...
        const getNewId = (oldId) => {
            if (!oldId)
                return null;
            if (!idMap.has(oldId))
                idMap.set(oldId, crypto_1.default.randomUUID());
            return idMap.get(oldId);
        };
        const plan = await prisma_1.default.floorPlan.create({
            data: {
                profileId: userId,
                name: name ?? 'My Floor Plan',
                description: description ?? null,
                floors: {
                    create: (floors ?? []).map((floor, idx) => ({
                        ...
                        rooms: {
                            create: (floor.rooms ?? []).map((room) => ({
                                ...
                                assignedUserIds: (room.assignedUserIds ?? []).map((uid) => getNewId(uid)),
                            })),
                        },
                        entryPoints: {
                            create: (floor.entryPoints ?? []).map((ep) => ({
                                id: getNewId(ep.id),
                                x: ep.x,
                                y: ep.y,
                                label: ep.label ?? null,
                        ...
                        evacuationRoutes: {
                            create: (floor.evacuationRoutes ?? []).map((er) => ({
                                id: getNewId(er.id),
                                points: er.points ?? [],
                            })),
                        },
...
router.put('/', async (req, res) => {
    try {
        ...
        const result = await prisma_1.default.$transaction(async (tx) => {
            ...
            await tx.floorPlanEvacuationRoute.deleteMany({ where: { floorLevel: { floorPlanId: id } } });
            await tx.floorPlanCorridor.deleteMany({ where: { floorLevel: { floorPlanId: id } } });
            await tx.floorPlanEntryPoint.deleteMany({ where: { floorLevel: { floorPlanId: id } } });
            await tx.floorPlanRoom.deleteMany({ where: { floorLevel: { floorPlanId: id } } });
            await tx.floorLevel.deleteMany({ where: { floorPlanId: id } });
            await tx.floorPlanUser.deleteMany({ where: { floorPlanId: id } });
            await tx.floorPlanAsset.deleteMany({ where: { floorPlanId: id } });
...
router.delete('/', async (req, res) => {
    try {
        ...
        await prisma_1.default.floorPlan.delete({ where: { id } });
